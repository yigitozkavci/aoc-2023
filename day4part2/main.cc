...
Card parse_card(const string &input)
{
  ...
  string card_header = splitted_input.at(0);
  int id = stoi(split(card_header, " ")[1]);
...
int num_of_matches(const Card &card)
{
  int result = 0;
...
int main()
{
  ...
  string input_line;
  int result = 0;
  unordered_map<int, int> scratch_copies;
  while (getline(input_file, input_line))
  {
    Card card = parse_card(input_line);
    if (scratch_copies.find(card.id) == scratch_copies.end())
    {
      scratch_copies[card.id] = 1;
    }
    int modifier = scratch_copies.at(card.id);
    for (int i = 1; i <= num_of_matches(card); i++)
    {
      int target_id = card.id + i;
      if (scratch_copies.find(target_id) != scratch_copies.end())
      {
        scratch_copies[target_id] += modifier;
      }
      else
      {
        scratch_copies[target_id] = 1 + modifier;
      }
    }
  }
  for (const auto &[k, v] : scratch_copies)
  {
    result += v;
  }
